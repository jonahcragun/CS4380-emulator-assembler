            ...
            match s:
                ...
                case "directive":
                    ...
                    if c.lower() == 'i':
                        ...
                        states["error"] = True
                # reading chars for str directive
                case "str0":
                    states[s] = False
                    if c.lower() == 't':
                        states["str1"] = True
                ...
                case "str2":
                    ...
                    if re.match(r'[ \t]', c):
                        ...
                        states["alloc_str_num"] = True
                    elif c == '"':
                        states["get_str_chars"] = True
                    else:
                        states["error"] = True
                # allocate specified number of bytes for a string
                case "alloc_str_num":
                    states[s] = False
                    if re.match(r'[\d]', c):
                        states[s] = True
                ...
                case "get_str_chars":
                    ...
                    if c != '"':
                        ...
                        states["alloc_str"] = True
                # read str value as chars from self.file, convert to ints, store in self.mem
                case "alloc_str":
                    states[s] = False
                    states["end_line"] = True
                    self.mem.append(len(value))
                    for i in value:
                        self.mem.append(ord(i))
                    self.mem.append(0)
                    value = ""
                    self.file = c + self.file
                ...
                case "byt2":
                    ...
                    if re.match(r'[ \t]', c):
                        ...
                        states["error"] = True
                # reading chars for bts dirctive
                case "bts1":
                    states[s] = False
                    if c.lower() == 's':
                        states["bts2"] = True
                ...
                case "alloc_bts":
                    ...
                    if re.match(r'[\d]', c):
                        ...
                        # Add 25 bytes to memory initialized to 0
                        if int(value) > self.MAX_INT or int(value) < self.MIN_INT:
                            states["error"] = True
                            continue
                        else:
                ...
                case State.OPERATOR_DONE:
                    if re.match(r'[ \t]', c):
                        ...
                        if operator in [Instr.JMP.name, Instr.CALL.name] and re.match(r'[a-zA-Z\d]', c):
                            s = State.JMP
                        elif operator in [Instr.MOV.name, Instr.ISTR.name, Instr.ILDR.name, Instr.ISTB.name, Instr.ILDB.name, Instr.IALLC.name]:
                            s = State.MOV
                        elif operator in [Instr.MOVI.name, Instr.ALCI.name]:
                            s = State.MOVI
                        elif operator in [Instr.LDA.name, Instr.STR.name, Instr.LDR.name, Instr.STB.name, Instr.LDB.name, Instr.BNZ.name, Instr.BGT.name, Instr.BLT.name, Instr.BRZ.name, Instr.ALLC.name]:
                            s = State.LD_ST
                        elif operator in [Instr.ADD.name, Instr.SUB.name, Instr.MUL.name, Instr.DIV.name, Instr.SDIV.name, Instr.CMP.name, Instr.AND.name, Instr.OR.name]:
                            s = State.ARITHMETIC
